...
unsigned IndexManager::getIndexPageFreeSpaceSize(const Attribute &attr, void * page) 
{
    ...
    return 4096 - free_space_pointer;
}
...
void IndexManager::split_index(IXFileHandle &ixfileHandle, const Attribute &attr, const unsigned de_size, const void *key, const unsigned leaf_ptr)
{
    ...
    if(index_ptr == 0){ // the root was split
        ...
        if (getIndexPageFreeSpaceSize(attr, parent_page) >= tc_size - sizeof(RID) + sizeof(unsigned)){ // parent has enough space
            unsigned second_page_index = (*ixfileHandle.get_handle()).getNumberOfPages();
                                 ...
                                 const void *key, const RID &rid, const unsigned parent_ptr, const unsigned ind_ptr, void *parent_page, void *page)
{
    ...
    if(leaf_dir.rptr != 0){
        /* get leaf to change its lptr */
        void *other_leaf_page = calloc(PAGE_SIZE, 1);
        if (other_leaf_page == NULL){
            free(second_page);
            return;
        }

        if ((*ixfileHandle.get_handle()).readPage(leaf_dir.rptr, other_leaf_page)){
            free(other_leaf_page);
            free(second_page);
            return;
        }
        
        unsigned new_lptr = (*ixfileHandle.get_handle()).getNumberOfPages();
        memcpy(other_leaf_page, &new_lptr, sizeof(unsigned));
        
        if ((*ixfileHandle.get_handle()).writePage(leaf_dir.rptr, other_leaf_page)){
            free(second_page);
            free(other_leaf_page);
            return;
        }
    ...
    leaf_dir.nentries = entry_count;
    leaf_dir.free_space_offset = offset;
    memcpy(page, &leaf_dir, sizeof(ldir));
...
void IndexManager::find_leaf(IXFileHandle &ixfileHandle, const Attribute &attribute, const void *key, const RID &rid, void *page, const unsigned nchildren, const unsigned parent_ptr, const unsigned next_level)
{
    ...
    /* put in last index ptr */
    unsigned ind_ptr;
    memcpy(&ind_ptr, (char *)page + offset - sizeof(unsigned), sizeof(unsigned));
...
RC IndexManager::delete_from_leaf(IXFileHandle &ixfileHandle, const Attribute &attr, const void *key, const RID &rid, void *page)
{   
    unsigned free_space_pointer;
    unsigned offset;
    if(levels != 0){ // non root leaf
        ldir leaf_dir = get_ldir(page);
    
        free_space_pointer = leaf_dir.free_space_offset;
    ...
    while(offset < free_space_pointer){
        ...
        switch(attr.type){
            case TypeInt:
                ...
                memcpy(&comp_rid, (char *)page + offset, sizeof(RID));
                if(comp_rid.pageNum == rid.pageNum && comp_rid.slotNum == rid.slotNum){
                    if(comp_rid.slotNum == 0){
                        comp_rid.slotNum = -413;
                    }else{
                        comp_rid.slotNum *= -1;
                    }
                    memcpy((char *)page + offset, &comp_rid, sizeof(RID));
                    return 0;
                }
                offset += sizeof(RID);
                break;
...
RC IndexManager::search_and_delete(IXFileHandle &ixfileHandle, const Attribute &attribute, const void *key, const RID &rid, const unsigned current_level, const unsigned current_ptr)
{   
    /* get page */
    void *page = malloc(PAGE_SIZE);
    if (page == NULL)
    ...
    if(current_level == levels || ixfileHandle.is_search == 1){ // this is a leaf page
        /* search leaf page and delete if found */
        if(delete_from_leaf(ixfileHandle, attribute, key, rid, page)){
            return 1;
        }
        
        if ((*ixfileHandle.get_handle()).writePage(current_ptr, page)){
            free(page);
            return 1;
        }
...
RC get_last_leaf(IXFileHandle &ixfileHandle, const unsigned index_ptr)
{
    /* get first leaf page */
    void *page = malloc(PAGE_SIZE);
    if (page == NULL)
    ...
    unsigned rptr;
    memcpy(&rptr, (char *)page + sizeof(unsigned), sizeof(unsigned));
    ...
    if(rptr == 0){
        ...
        return get_last_leaf(ixfileHandle, rptr);
    }
...
RC IndexManager::insertEntry(IXFileHandle &ixfileHandle, const Attribute &attribute, const void *key, const RID &rid)
{
    ...
    if(nchildren != 0){ // the root contains index node pointers
        ...
        if (getRootPageFreeSpaceSize(pageData) >= deSize){ // root has enough space
            ...
            cout << "----------------------init_leaves_from_root---------------------" << endl;
        ...
        IX_ScanIterator &ix_ScanIterator)
{
	...
	ix_ScanIterator.startFlag = true;
	ix_ScanIterator.attribute = attribute;	

	int nodeNum;
	
	bool root_is_leaf = (ixfileHandle.get_handle())->getNumberOfPages() == 1;

	if (lowKey != NULL) {

		nodeNum = search(ixfileHandle, attribute, lowKey, 0, 0);
		ix_ScanIterator.currentPageNum = nodeNum;
	}
	
	if(lowKey == NULL && root_is_leaf == false){
	    ix_ScanIterator.currentPageNum = 1;
	}


	if (highKey != NULL) {
		nodeNum = search(ixfileHandle, attribute, highKey, 0, 0);
		/*
         	void *lastPage = malloc(PAGE_SIZE);
	        memset(lastPage, 0, PAGE_SIZE);
	        if((*ixfileHandle.get_handle()).readPage(nodeNum, lastPage)){
		       free(lastPage);
		       return -1;
	        }
	        ldir leaf_dir = get_ldir(lastPage);
		*/
		ix_ScanIterator.lastPageNum = nodeNum;
	}

    if(highKey == NULL && root_is_leaf == false){
	    ix_ScanIterator.lastPageNum = get_last_leaf(ixfileHandle, 1);
	}
	...
	if (attribute.type == TypeVarChar) {
		lowKeySize = sizeof(int) + *(int*) lowKey;
		highKeySize = sizeof(int) + *(int*) highKey;

	} else if (attribute.type == TypeInt) {
		lowKeySize = sizeof(int);
		highKeySize = sizeof(int);
	} else {
		lowKeySize = sizeof(float);
		highKeySize = sizeof(float);
	}

	if (lowKey != NULL) {
		ix_ScanIterator.lowKey = malloc(lowKeySize);
		memcpy(ix_ScanIterator.lowKey, lowKey, lowKeySize);
	} else {
		ix_ScanIterator.lowKey = NULL;
	}

	if (highKey != NULL) {
		ix_ScanIterator.highKey = malloc(highKeySize);
		memcpy(ix_ScanIterator.highKey, highKey, highKeySize);
	} else {
...
unsigned IndexManager::search(IXFileHandle &ixfileHandle, const Attribute &attribute, const void *key, const unsigned current_level, const unsigned current_ptr)
{   
    /* get page */
    void *page = malloc(PAGE_SIZE);
    if (page == NULL)
        ...
        switch(attribute.type){
            case TypeInt:
                ...
                if(key_data < num_data){
                    ...
                    return search(ixfileHandle, attribute, key, current_level + 1, index_ptr);
                }
                offset += 2 * sizeof(unsigned);
                break;
            case TypeReal:
    ...
    // do we need to free page here?    
    //free(page);
    return search(ixfileHandle, attribute, key, current_level + 1, index_ptr);
}

//------------------------------------------------------------------------
int compareValues(const void * value1, void * value2, const Attribute &attribute){
...
RC IX_ScanIterator::getNextEntry(RID &rid, void *key)
{
	...
	unsigned flag = 1;
	if((ixfileHandle->get_handle())->getNumberOfPages() == 1){
	    flag = 0;
	}
	 ...
	 ldir leaf_dir;
    if(flag == 1){
	     leaf_dir = get_ldir(startPage);
	     nentries = leaf_dir.nentries;
	 }else{
	     memcpy(&nentries, (char *)startPage + sizeof(idir), sizeof(unsigned));
	 }
     int i;
	if (startFlag) {
		 for(i = 0; i < nentries; i++){
			 get_leaf_entry(startPage, attribute, flag, i, rid, key);

			 if ( (lowKeyInclusive && compareValues(key, lowKey, attribute) >= 0)
			     || (compareValues(key, lowKey, attribute) > 0)) {
				if ((highKey == NULL) && ((highKeyInclusive && compareValues(highKey, key, attribute) <= 0)
				    || (compareValues(highKey, key, attribute) < 0))) {
					// rid and key are already set.
					break;
				}
				if ((highKey != NULL) && ((highKeyInclusive && compareValues(key, highKey, attribute) <= 0)
				    || (compareValues(key, highKey, attribute) < 0))) {
					// rid and key are already set.
					break;
				}
			 }
		 ...
		 currentEntryNum = i;
		 if(++currentEntryNum > nentries - 1) {
			if (currentPageNum == lastPageNum) {
				endOfFile = true;
			}

			currentEntryNum = 0;
			currentPageNum = leaf_dir.rptr;
		 }
		startFlag = false;
		return 0;
	}
	
	get_leaf_entry(startPage, attribute, flag, currentEntryNum, rid, key);
	if ((highKey != NULL) && ((highKeyInclusive && compareValues(key, highKey, attribute) <= 0)
	    || (compareValues(key, highKey, attribute) < 0))) {
		// We have already key and rid. So we don't need to do anything.
					//cout << " GEEETS HEERERE 3" << endl;
	} else if ((highKey == NULL) && ((highKeyInclusive && compareValues(highKey,key,  attribute) <= 0)
		|| (compareValues(highKey, key, attribute) < 0))) {
				//	cout << " GEEETS HEERERaE 4" << endl;
