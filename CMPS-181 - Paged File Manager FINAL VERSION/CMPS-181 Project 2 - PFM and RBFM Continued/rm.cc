...
{   
    ...
    num_tables = _rbf_manager->getNumTables(handle);
...
{
    if(!fileExists(tableName) || tableName == "Tables" || tableName == "Columns"){
        return -1;
    }
    ...
    scan_tab.scanned_file = "Tables";
    
    if(scan_tab.scan_result.size() > 0){
        ...
        if(deleteTuple("Tables", scan_tab.scan_result.at(0)))
            return -1;
        ...
        scan_col.scanned_file = "Columns";
    
        if(_rbf_manager->closeFile(handle))
            return -1;
    
        for(unsigned i = 0; i < scan_col.scan_result.size(); ++i){
            if(deleteTuple("Columns", scan_col.scan_result.at(i)))
                return -1;
        }
...
RC RelationManager::getAttributes(const string &tableName, vector<Attribute> &attrs)
{
    ...
    //cout << scan_tab.scan_result[0].slotNum << endl;
...
RC RelationManager::insertTuple(const string &tableName, const void *data, RID &rid)
{
    ...
    if(_rbf_manager->insertRecord(handle, attrs, data, rid))
        return -1;
...
RC RelationManager::deleteTuple(const string &tableName, const RID &rid)
{
    ...
    if(_rbf_manager->deleteRecord(handle, attrs, rid))
        return -1;
...
RC RelationManager::updateTuple(const string &tableName, const void *data, const RID &rid)
{
    ...
    if(_rbf_manager->updateRecord(handle, attrs, data, rid))
        return -1;
...
RC RelationManager::readTuple(const string &tableName, const RID &rid, void *data)
{
    ...
    if(_rbf_manager->readRecord(handle, attrs, rid, data))
        return -1;
...
RC RelationManager::readAttribute(const string &tableName, const RID &rid, const string &attributeName, void *data)
{
    ...
    if(_rbf_manager->readAttribute(handle, attrs, rid, attributeName, data))
        return -1;
    
    for(unsigned i = 0; i < attrs.size(); ++i){
        if(attrs[i].name == attributeName){
            attr_size = attrs[i].length;
        }
    }
        
    memmove((char *)data + 1, data, attr_size);
    memset(data, 0, 1);
    if(((char *)data)[0] == 'N' && ((char *)data)[1] == 'U' && 
       ((char *)data)[2] == 'L' && ((char *)data)[3] == 'L' && 
       ((char *)data)[4] == '\0'){
        ((char *)data)[0] ^= 128;
    }
      ...
      RM_ScanIterator &rm_ScanIterator)
{
    ...
    if (_rbf_manager->scan(handle, attrs, conditionAttribute, compOp, value, attributeNames, rbfmsi))
	return -1;

    rbfmsi.scanned_file = tableName;
    rm_ScanIterator.setRbfmScanIterator(rbfmsi);
...
RC RM_ScanIterator::getNextTuple(RID &rid, void *data) {
    ...
    if(pos >= iter.scan_result.size()){
        return RM_EOF;
    }
    iter.getNextRecord(iter.scan_result[pos], data);
    pos++;
    
    unsigned attr_size = 0;
    for(unsigned i = 0; i < iter.recordDescriptor.size(); ++i){
        for(unsigned k = 0; k < iter.attr_names.size(); ++k){
            if(iter.recordDescriptor[i].name == iter.attr_names[k]){
                attr_size += iter.recordDescriptor[i].length;
                break;
            }
        }
    }
    int new_null_size = int(ceil((double) iter.attr_names.size() / CHAR_BIT));
    memmove((char *)data + new_null_size, data, attr_size);
    memset(data, 0, new_null_size);
