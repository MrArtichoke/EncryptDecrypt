...
public:
  FileHandle iter_handle;
  vector<Attribute> attr_names;
  RID pos;

  RBFM_ScanIterator() { pos.pageNum = 0; pos.slotNum = 0; iter_handle.setfd(NULL); };
  ~RBFM_ScanIterator() {};
  ...
  RC close();
  void get_projected_at_offset(void *page, unsigned offset, const vector<Attribute> &recordDescriptor, void *data);
};
...
private:
  ...
  int scan_attrs(FileHandle &fileHandle, 
                const vector<Attribute> &recordDescriptor, 
                const string &conditionAttribute, const CompOp compOp,
                const void *value, const vector<string> &attributeNames,
                const RID &rid);
            
 void prepare_iter_record(const vector<string> &attrs, const vector<Attribute> &recordDescriptor, const void *record, void *data);
                
  RC insertRecord_no_slot(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, RID &rid);
