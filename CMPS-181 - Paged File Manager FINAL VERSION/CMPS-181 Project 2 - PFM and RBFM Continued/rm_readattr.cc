...
RC RelationManager::getAttributes(const string &tableName, vector<Attribute> &attrs)
{
    ...
    void *table_id_data = malloc(5);
    void *table_name_scan = malloc(tableName.length() + 1);
    memset(table_name_scan, 0, 1);
    memcpy((char *)table_name_scan + 1, tableName.c_str(), tableName.length());
    
    if(_rbf_manager->openFile("Tables", handle))
        return -1;
    
    if(_rbf_manager->scan(handle, tAttributes, "table-name", EQ_OP, table_name_scan, attr_names, scan_tab))
        return -1;
        
    if(_rbf_manager->readAttribute(handle, tAttributes, scan_tab.scan_result.at(0), "table-id", table_id_data))
        return -1;
    ...
    if(_rbf_manager->scan(handle, cAttributes, "table-id", EQ_OP, table_id_data, attr_names, scan_col))
        return -1;
