...
void populate_attrs(vector<Attribute> &attrs, RBFM_ScanIterator &scan_col)
{
    ...
    while(scan_col.getNextRecord(rid, data)){
        unsigned offset = 1;
...
RC RelationManager::getAttributes(const string &tableName, vector<Attribute> &attrs)
{
    ...
    RID rid;
    void *data = malloc(5);
    scan_tab.getNextRecord(rid, data);
    ...
    if(_rbf_manager->scan(handle, cAttributes, "table-id", EQ_OP, (char *)data + 1, attr_names, scan_col))
        return -1;
...
{
    // fallback: create the iterator file in here
    return -1;
}