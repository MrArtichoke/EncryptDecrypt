...
RC RecordBasedFileManager::insertRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, RID &rid) 
{
    ...
    /*for(unsigned l = 0; l < slotHeader.recordEntriesNumber; ++l){
         SlotDirectoryRecordEntry checkRecordEntry =  getSlotDirectoryRecordEntry(pageData, l);
         if(checkRecordEntry.length < 0){
             rid.slotNum = l;
             break;
         }
    }*/
...
RC RecordBasedFileManager::deleteRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const RID &rid)
{
    ...
    if(record_entry.offset < 0){
        RID new_rid;
        new_rid.pageNum = record_entry.length;
        if(record_entry.offset != -413){
            new_rid.slotNum = record_entry.offset * -1;
        }else{
            new_rid.slotNum = 0;
        }
    ...
    if(slot_header.freeSpaceOffset != record_entry.offset){ // record to be deleted is not in the last slot
        ...
        for(unsigned i = 0; i < slot_header.recordEntriesNumber; ++i){
            SlotDirectoryRecordEntry update_offsets = getSlotDirectoryRecordEntry(page_data, i);
            if(update_offsets.offset < 0 || update_offsets.length < 0){
                continue;
            }
            if(update_offsets.offset < record_entry.offset){
...
RC RecordBasedFileManager::updateRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, const RID &rid)
{
    ...
    if(getPageFreeSpaceSize(page_data) < recordSize){ // record too big for page
        ...
        //cout << record_entry.length << " " << record_entry.offset << endl;
...
unsigned RecordBasedFileManager::getNumTables(FileHandle &fileHandle){
    void *page_data = malloc(PAGE_SIZE);
    if (fileHandle.readPage(0, page_data)){
        free(page_data);
        return RBFM_READ_FAILED;
    }
    ...
    free(page_data);
    return slot_header.recordEntriesNumber;
}
...
RC RBFM_ScanIterator::getNextRecord(RID &rid, void *data)
{
    ...
    FILE *file = handle.getfd();
    if(file == NULL){
        FILE *pFile;
        pFile = fopen(scanned_file.c_str(), "rb+");
        // If we fail, error
        if (pFile == NULL){
            free(pageData);
