...
RC RecordBasedFileManager::readAttribute(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const RID &rid, const string &attributeName, void *data)
{
    ...
    if(is_varchar){
        memcpy((char *)data, (char *)record + (offset - 4), 4);
        memcpy((char *)data + 4, (char *)record + offset, varchar_len);
    }else{
...
int compare_attr(const void *attr, const CompOp compOp, const void *value, const int cmp_t, const unsigned size){
    ...
    int memcmp_val;
    if(cmp_t == 0){
        memcpy(&int_attr, ((char*) attr), INT_SIZE);
        memcpy(&int_value, ((char*) value), INT_SIZE);
    }
    if(cmp_t == 1){
        memcpy(&real_attr, ((char*) attr), REAL_SIZE);
        memcpy(&real_value, ((char*) value), REAL_SIZE);
    }
    if(cmp_t == 2){
        unsigned str_size;
        memcpy(&str_size, attr, 4);
        memcmp_val = memcmp((char *)attr + 4, value, str_size);
    }
...
void RecordBasedFileManager::prepare_iter_record(const vector<string> &attrs, const vector<Attribute> &recordDescriptor, const void *record, void *data)
{
    unsigned offset = 0;
    unsigned num_rec_fields = recordDescriptor.size();
    
    // set null indicator
    int rec_nullIndicatorSize = getNullIndicatorSize(num_rec_fields);
    char rec_null_ind[rec_nullIndicatorSize];
    memset(rec_null_ind, 0, rec_nullIndicatorSize);
    memcpy(rec_null_ind, record, rec_nullIndicatorSize);
    
    unsigned rec_offset = rec_nullIndicatorSize;
    
    int nullIndicatorSize = getNullIndicatorSize(attrs.size());
    char new_null_ind[nullIndicatorSize];
    memset(new_null_ind, 0, nullIndicatorSize);
    
    unsigned j = 0;
    for(unsigned i = 0; i < num_rec_fields; ++i){
        if(fieldIsNull(rec_null_ind, i)){
            if(i == 0){
                new_null_ind[j] ^= 1;
            }else{
                unsigned power = pow(2, i);
                new_null_ind[j] ^= power;
            }
        }
    ...
    memcpy((char *)data + offset, &new_null_ind, nullIndicatorSize);
    offset += nullIndicatorSize;
    
    // set field values
    unsigned is_projected = 0; // 0 = false, 1 = true
    unsigned char_len = 0;
    for(unsigned i = 0; i < num_rec_fields; ++i){
        if(fieldIsNull(rec_null_ind, rec_nullIndicatorSize)){
            continue;
        }
        
        if(recordDescriptor[i].type == TypeVarChar){
            memcpy(&char_len, (char *)record + rec_offset, 4);
        }
        
        for(unsigned j = 0; j < attrs.size(); ++j){
            if(attrs[j] == recordDescriptor[i].name){
                is_projected = 1;
            }
        }
        ...
        if(is_projected == 1){
            if(recordDescriptor[i].type == TypeVarChar){
                memcpy((char *)data + offset, (char *)record + rec_offset, 4);
                offset += 4;
                rec_offset += 4;
                
                memcpy((char *)data + offset, (char *)record + rec_offset, char_len);
                offset += char_len;
                rec_offset += char_len;
            }else{
                memcpy((char *)data + offset, (char *)record + rec_offset, 4);
      ...
      RBFM_ScanIterator &rbfm_ScanIterator)
{
    ...
    if(fileExists("scan_result")){
       destroyFile("scan_result"); 
    }
    
    createFile("scan_result");
    FileHandle handle;
    openFile("scan_result", handle);
    
    unsigned record_size = 0;
    unsigned data_size = 0;
    unsigned num_found = 0;
    for(unsigned j = 0; j < recordDescriptor.size(); ++j){
        record_size += recordDescriptor[j].length;
        if(j % 8 == 0){
            record_size++;
        }
        ...
        for(unsigned i = 0; i < attributeNames.size(); ++i){
            if(attributeNames[i] == recordDescriptor[j].name){
                data_size += recordDescriptor[j].length;
                if(num_found % 8 == 0){
                    data_size++;
                }
                num_found++;
                rbfm_ScanIterator.attr_names.push_back(recordDescriptor[j]);
            }
        }
    }
    ...
    for(curr_page = 0; curr_page < num_pages; ++curr_page){
        ...
        for(curr_record = 0; curr_record < slot_header.recordEntriesNumber; ++curr_record){
            ...
            if(scan_val != 0){ // record fits criteria
                ...
                prepare_iter_record(attributeNames, recordDescriptor, record, data);
                
                insertRecord(handle, recordDescriptor, data, iter_rec_rid);
...
{
    if(iter_handle.getfd() == NULL){
        //openFile("scan_result", iter_handle);
        FILE *pFile;
        pFile = fopen("scan_result", "rb+");
        // If we fail, error
        if (pFile == NULL)
            return PFM_OPEN_FAILED;

        iter_handle.setfd(pFile);
    }
    
    void * pageData = malloc(PAGE_SIZE);
    if (iter_handle.readPage(pos.pageNum, pageData))
        return RBFM_READ_FAILED;

    // Checks if the specific slot id exists in the page
    SlotDirectoryHeader slotHeader = getSlotDirectoryHeader(pageData);
    if(slotHeader.recordEntriesNumber <= pos.slotNum){
        if(pos.pageNum + 1 >= iter_handle.getNumberOfPages()){
            return RBFM_EOF;
        }
    ...
    // Gets the slot directory record entry data
    SlotDirectoryRecordEntry recordEntry = getSlotDirectoryRecordEntry(pageData, pos.slotNum);

    // Retrieve the actual entry data
    getRecordAtOffset(pageData, recordEntry.offset, attr_names, data);

    free(pageData);
    rid.pageNum = pos.pageNum;
    rid.slotNum = pos.slotNum;
    pos.slotNum++;
    return SUCCESS;
}
